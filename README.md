...
```bash
git clone https://github.com/alanchenchen/suna.git
cd suna
go install .
suna
```
...
构建产物默认放在 `dist/`。压缩包名称带平台/架构后缀（例如 `suna-darwin-arm64.zip`），但包内二进制统一为 `suna`（Windows 为 `suna.exe`），便于解压后直接放入 `PATH`。Darwin、Linux、Windows 产物不能混用，需要按目标系统选择对应包。
...
常用设置都可以在 TUI 中通过 `/config` 修改，不必手动编辑配置文件。`context_window` 和 `max_output_tokens` 是模型能力参数，必须按当前模型服务实际限制填写。`strengths` 用于告诉主 Agent 模型擅长什么；`subtask_for` 是可选的子任务模型可见性过滤器，留空表示所有主模型都可使用，支持 `provider/model` glob（例如 `DF/**`、`Froghire/gpt-*`），且模型始终可作为自己的子任务模型。
