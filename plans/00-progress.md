...
- Skill 已有可用 MVP：`~/.suna/skills`、`skill.load`、`skill.start import/check`、enabled/reasons 和 `/skills` overlay 已闭环；MCP runtime 仍待实现。
- Trigger、MCP runtime、hooks、trust、reflection、sandbox、marketplace 基本是预留或设计。
...
| Skill / MCP | Skill Usable MVP / MCP Design | Skill runtime 已闭环：`~/.suna/skills`、enabled/reasons、`skill.load`、`skill.start` 固定验收流程、`/skills` overlay；MCP runtime 待实现 | 稳定 Skill 边界与文档，后续实现 MCP stdio |
...
- 对话导入或新建 Skill 时走 `skill.start import/check` 固定流程：static check → 询问是否 LLM review → 询问是否启用。
- prompt 中注入 active skill index，LLM 按需调用 `skill.load(name)` 加载完整 `SKILL.md`。
...
- Skill check 只在 `skill.start` workflow 中执行，能输出 `reasons` 并写入 config。
