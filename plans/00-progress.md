...
- Skill 已有可用 MVP：`~/.suna/skills`、`skill_load`、`skill_start import/check`、enabled/reasons 和 `/skills` overlay 已闭环；MCP runtime 仍待实现。
...
| Skill / MCP | Skill Usable MVP / MCP Design | Skill runtime 已闭环：`~/.suna/skills`、enabled/reasons、`skill_load`、`skill_start` 固定验收流程、`/skills` overlay；MCP runtime 待实现 | 稳定 Skill 边界与文档，后续实现 MCP stdio |
