...
- Skill/MCP 方案：通用 Agent Skill、导入/check/启用、`skill.load` 和 MCP 独立配置；runtime 待实现。
...
| Skill / MCP | Design | 通用 Skill + MCP 独立配置已定稿，runtime 待实现 | 实现 SkillManager、skill.load、check 和 MCP stdio |
...
- `config.toml` 只记录 `[skills.<name>] enabled/hash/reasons`。
- 导入、生成、内容变化时执行 check，并让用户决定是否启用。
- prompt 中注入 active skill index，LLM 按需调用 `skill.load(name)` 加载完整 `SKILL.md`。
- MCP server 独立放在 `config.toml [mcp.servers.*]`，不放进 Skill 包。

完成标准：

- 未 check / hash 变化 / disabled 的 Skill 不进入上下文。
- 用户能在 chat 顶部或 `/skills` 中看到 active / unchecked / needs_review。
- Skill check 能输出 `reasons` 并写入 config。
