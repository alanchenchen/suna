...
当前 TUI 的 `agent.delta`、`agent.run`、`agent.tool_start`、`agent.tool_guard`、`agent.tool_end`、`agent.ask_user`、`agent.guard_confirm` 等事件也应归入 `protocol`，因为它们是 daemon 对外一致事件流，不是 local transport 私有事件。
...
```
agent.delta          {kind, content}        // LLM 内容增量，kind=assistant|reasoning
agent.run            {state, phase?, error?, resume_available?} // run 生命周期、retry 和最终错误
...
```
格式: NDJSON (每行一条 JSON，\n 分隔)
  {"jsonrpc":"2.0","method":"agent.delta","params":{"kind":"assistant","content":"你"}}
  {"jsonrpc":"2.0","method":"agent.delta","params":{"kind":"assistant","content":"好"}}
...
写阻塞保护:
  ...
  - daemon 对 `agent.delta` 做短周期 micro-batching，减少 IPC/TUI 事件数量
  - 文本 delta 不丢弃；tool/usage/run/error 等关键事件前会先 flush pending 文本
```
