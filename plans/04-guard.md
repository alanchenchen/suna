...
每个被 Guard 覆盖的工具都会在执行前发出 `EventToolGuard`，daemon 转成 `agent.tool_guard`，字段包含 `tool_call_id/tool/risk/decision/source/reason/suggestion`。`source` 取值为 `static`、`rule`、`llm`、`user`、`fallback`，用于客户端展示“谁批准/阻止了这次操作”。用户确认后会再次发送 `agent.tool_guard` 覆盖前置 LLM/fallback 状态，保留最终用户决策。

### Guard Confirm 流程

```
1. Guard.Check() 返回 "confirm"
2. Core 先发送 EventToolGuard，TUI 在工具块下显示 LLM/fallback 的前置决策
3. Core 通过 EventGuardConfirm 暂停 tool 执行
4. IPC 发送 NotifyGuardConfirm 到 TUI
5. TUI 渲染 Guard confirm overlay
6. 用户通过 `Y/A` approve、`N/R/Esc` reject，或方向键选择后 `Enter` 确认
7. TUI 通过 MethodGuardReply 回传决策
8. Core 通过 Reply chan 收到决策，并发送 source=user 的 EventToolGuard 覆盖最终状态
9. 执行或拒绝 tool
```
