...
| 流式回答 | 已实现 | `events/`, `events.go`, `chat.go` | `agent.delta kind=assistant` 追加 assistant 内容；高频 delta 由 event batcher 合并 |
...
模型或上游服务错误导致 run 停止时，daemon 通过 `agent.run state=failed` 暴露结构化 `ModelError` 和 `resume_available=true/false`。TUI 不解析错误文本；只有 `resume_available=true` 时才显示输入区提示。此时输入框为空按 `Enter` 调 `agent.resumeRun`，输入了新消息则继续走 `agent.sendMessage`。
...
- TUI 提供 `Clear Reasoning`、`GPT`、`Claude`、`DeepSeek V4`、`MiniMax M3`、`Custom Reasoning`。Claude preset 靠近 Claude Code 风格：Disabled、Think、Think Hard、Think Harder、Ultrathink，并固定映射到 Anthropic `thinking.budget_tokens`。
...
| `agent.delta` | assistant/reasoning 内容增量，`kind=assistant|reasoning` |
| `agent.run` | run lifecycle：running/retrying/done/failed/cancelled，携带结构化 error 和 resume availability |
