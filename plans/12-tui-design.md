...
选择后进入 provider form。`openai` 固定表示 OpenAI 官方 Responses API；`anthropic` 表示 Anthropic Messages API；`openai-compatible` 会让用户填写自定义 provider ID 和 Chat Completions endpoint。`openai` 和 `anthropic` 会填入默认 endpoint 提示，`anthropic` context window placeholder 为 `200000`。
...
| 字段 | 状态 | 说明 |
|---|---|---|
| Provider | 必填 | `openai`/`anthropic` 是保留 provider ID；其他名称按 OpenAI-compatible Chat Completions provider 处理 |
| Model | 必填 | 实际模型 ID |
| API Key | setup 必填，编辑可留空 | 密码回显；留空表示不修改已有凭证 |
| Endpoint | openai/anthropic 可空，openai-compatible 必填 | 非空时校验 URL；仅自定义 Chat Completions provider 实际使用该字段 |
