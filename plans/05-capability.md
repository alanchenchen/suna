...
后续 MCP 也应作为 `internal/tools/mcptools.Provider` 接入同一套工具目录和执行路由。
...
env              显式传给 stdio server 的环境变量
url              SSE / Streamable HTTP endpoint
headers          远程 transport 请求头，可选
```
...
### Phase 1 — stdio tools-only MVP

目标：能连接常见本地 stdio MCP server，并把 MCP tools 暴露给模型。
...
支持手动 reload/restart MCP servers
配置变更后重新加载 MCP runtime 和 tools.Manager
```
