...
- 默认使用标准 `testing` 包，不为普通测试引入 assertion framework。
- 测试文件放在被测包旁边；内部包优先使用同包测试，只有刻意验证公开 API 时才使用外部测试包。
- 测试按行为命名；同类行为优先使用以 `name` 开头的表驱动用例，并通过 `t.Run` 执行。
- Go 1.22+ 已修正循环变量语义，不需要在表驱动测试中重复写 `tt := tt`。
- 失败信息使用 `got` / `want`，默认通过 `t.Fatal` / `t.Fatalf` 快速失败。
- 测试 helper 必须调用 `t.Helper()`；文件、环境变量和外部依赖分别使用 `t.TempDir()`、`t.Setenv()` 和 fake/stub 隔离。
- 测试文件不写清单式文件头注释；解释性注释只说明“为什么”，不复述测试步骤。
- 普通 `go test ./...` 必须快速、离线、确定性；需要 daemon、网络、真实模型或其它外部依赖的测试使用 `integration` build tag。
...
TUI 与 daemon 的交互必须通过 protocol 和 transport；第三方客户端通过 `suna serve --json` 获取 TCP endpoint 后也必须遵守同一套 protocol。
...
- `docs/` 记录当前稳定架构、实现事实和开发约定。
- `plans/` 只记录面向未来的长期方向、待验证假设和演进原则，不保存已经完成或失效的阶段性计划。
