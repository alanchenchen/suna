...
本文集中记录 Suna 当前已经实现的性能优化。它只描述当前代码事实，不把 `plans/` 中的未来方向或待验证假设当作已完成能力。
...
`session_state.tool_summary` 不是工具历史，也不参与模型上下文。daemon 只维护一个有界结构化摘要：累计 total / success / failed，最多保留最近 4 条工具名、最多 2 条失败摘要，以及变更类工具计数。session attach/create snapshot 会携带该结构，TUI 根据 locale 渲染文案；daemon 不生成中文/英文展示字符串。

相关代码：

- `internal/memory/tool_summary.go`
...
- `read_image` 受 `media.Store.ValidateImage` 约束（MIME 白名单、10MB 上限、attachment 目录约束、symlink 拒绝）；base64 只在模型请求阶段生成并立即回收，图片块只活一轮，run 结束时替换为摘要文本，重复读图不重复注入。
...
- `internal/tools/builtin/read_image.go`
...
assistant / reasoning streaming 阶段使用传输级 micro-batching；runner 内部用 append-only builder 累计最终文本，避免长回复中 `fullContent += chunk` 造成反复复制。TUI 侧再使用轻量 wrap + indent，不高频运行 Glamour Markdown。收到 done 后清除 streaming 标记，再对最终 assistant 内容做完整 Markdown 渲染。
...
当前 multi-session 版本采用 **单 daemon、全局 runtime、多 session 状态隔离**：DB 中存在的历史 session 不会自动加载为内存 runtime；只有本次 daemon 生命周期内被 attach 过的 session 才会创建 in-memory runtime。空 session detach 后会按生命周期规则清理；非空 idle session 的 runtime 当前会作为热缓存保留，后续 attach 可直接复用。

这些风险是当前设计下的已知边界，不是提交阻塞项：

- **idle runtime 热缓存增长**：长期运行 daemon 且打开过大量非空 session 时，`sessionRuntime`、per-session agent、working memory、media store 和 guard/run 状态会增加常驻内存。正常几十个 session 风险较低；几百/上千个被打开过的 session 可能需要后续做 idle runtime LRU / unload。
- **无全局 run limiter**：不同 session 允许并行运行。多个 session 同时调用模型、工具、subtask 或 MCP server 时，CPU、网络、磁盘 IO 和模型请求压力会随 active run 数线性上升。这是 v1 的设计选择。
- **running buffer 峰值**：为了 Join active session，daemon 在运行中保存轻量 `CurrentRunView` buffer，包括 assistant / reasoning 当前输出。超长回复或多个长 run 并发时会增加短期内存峰值；run idle 后会清理。
- **broadcast fan-out**：同一 session 多个 client attach 时，stream、tool、run、usage 等事件会按 client fan-out。Handoff 目标是少量窗口接力/观察，不按多人协同会议场景优化。
- **TUI 长输出渲染成本**：Chat 已有窗口化渲染、增量 wrap cache 和有界 Markdown cache，但超长输出、多个 TUI 观察同一长 run 时仍会增加终端渲染压力。
- **SQLite 写入竞争**：多 session 并行结束、compact 或写 memory 时共享同一个 SQLite，可能出现写锁等待或短时抖动。普通使用下风险较低。
- **MCP / tools 全局瓶颈**：tools manager 和 MCP runtime 是 daemon-global；多个 session 同时调用同一 MCP server 时，瓶颈取决于该 server 的并发能力。
- **附件目录 IO 峰值**：`attachment.clear` 和 `session.delete` 会删除当前 session 的附件目录；大量大文件会造成短时磁盘 IO。
- **session.list 无分页**：v1 按设计不分页。几十/几百个 session 可接受；上千/上万个 session 会增加 DB 查询、JSON payload 和 TUI 列表渲染成本。

后续优先考虑的优化：

1. **idle session runtime eviction**：对 `client_count=0`、`status=idle`、状态已持久化且一段时间未使用的 session 卸载内存 runtime，下次 attach 再从 DB 恢复。
2. **可配置全局并发上限**：限制同时 active 的 session run 数，避免误操作把本机、MCP server 或模型额度打满。
