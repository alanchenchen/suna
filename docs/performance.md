...
`conversation_state.tool_summary` 不是工具历史，也不参与模型上下文。daemon 只维护一个有界结构化摘要：累计 total / success / failed，最多保留最近 4 条工具名、最多 2 条失败摘要，以及变更类工具计数。会话恢复时该结构通过 `session.restore_status.tool_summary` 下发，TUI 根据 locale 渲染文案；daemon 不生成中文/英文展示字符串。
