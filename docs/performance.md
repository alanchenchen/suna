...
- **长历史不全量重放**：模型上下文使用 Session State + recent window，TUI transcript 使用 visible window，不把完整历史无限塞给模型或 viewport。
...
### Chat transcript windowed rendering

长历史 Chat transcript 使用窗口化渲染：
...
该设计参考 Bubbles `table` / `list` 的窗口化思路：完整数据留在业务 model，View 只渲染当前窗口，而不是把无限增长的历史交给 viewport。
...
transcript viewport window 使用内容签名：window start/end、yOffset、width/height、total lines 和 visible lines hash 完全一致时，跳过重复 `SetContentLines`。这不是帧率限制，也不是 stream 节流；Bubble Tea 仍正常处理输入和事件，只是不重复塞同一份内容给 viewport。
