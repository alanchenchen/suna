...
- session restore 只把真实 user/assistant 作为 `session.restore_message` 追加到 transcript；结构化工具摘要通过 `session.restore_status.tool_summary` 下发，由 TUI 根据 locale 渲染成本地化 restore summary block，不要求 daemon 生成展示文案。
