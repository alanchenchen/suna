...
func TestEditFileAppliesMultipleEditsAtomically(t *testing.T) {
	...
	res := EditFile{}.Execute(context.Background(), map[string]any{
		...
		"edits": []any{
			...
			map[string]any{"old_string": "beta", "new_string": "BETA", "occurrence": float64(2), "expected_replacements": float64(1)},
		},
	})
	if res.IsError {
...
func TestEditFileRequiresOccurrenceForAmbiguousEdit(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.txt")
	...
	if !res.IsError || !strings.Contains(res.Error, "found 2 times") {
		t.Fatalf("EditFile.Execute() = %#v, want ambiguous match error", res)
	}
}
